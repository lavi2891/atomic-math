...
function amGetStudentHome_(payload){var ss=amSpreadsheet_();var id=String(payload.studentId||"").trim().toUpperCase();var students=amObjects_(ss.getSheetByName("Students"));var student=students.filter(function(s){return String(s.studentId).trim().toUpperCase()===id;})[0]||null;var active=!!student&&(student.active===true||String(student.active).toUpperCase()==="TRUE");return{student:active?student:null,studentStatus:student?(active?"active":"inactive"):"unknown",activeAssignments:active?amObjects_(ss.getSheetByName("Assignments")).filter(function(a){return String(a.studentId).trim().toUpperCase()===id&&(a.active===true||String(a.active).toUpperCase()==="TRUE");}):[],masterySnapshots:active?amObjects_(ss.getSheetByName("Mastery")).filter(function(m){return String(m.studentId).trim().toUpperCase()===id;}):[]};}
