...
const NUMBER_LINE_CONCEPT_GENERATOR = conceptualGenerator({
  id: "MVP_INT_NUMBER_LINE_DIRECTION_A", skillId: "INT_NUMBER_LINE", family: "left-move-decreases-value", category: "conceptual", band: "A", exprTemplate: "{n}-1<{n}", params: { wordingVariant: { type: "integer", min: 0, max: 2 }, n: { type: "integer", min: -10, max: 10 } }, difficultyFeature: "structure", structuralStage: "direction-value-invariant", version: 6,
  choiceBuilder: (params) => { const n = sampledInteger(params, "n"); return generatedChoiceDraft("INT_NUMBER_LINE", sampledWording(params, [`מתחילים ב־[[${n}]] ונעים צעד אחד שמאלה על ציר המספרים. מה נכון תמיד?`, `נקודה נמצאת ב־[[${n}]]. מזיזים אותה יחידה אחת שמאלה. איזה יחס נכון?`, `על ציר המספרים עוברים מ־[[${n}]] אל הנקודה שמשמאלו. מה קורה לערך?`]), [{ value: `המספר החדש קטן מ־[[${n}]]`, correct: true, misconception: "left-move-decreases-value" }, { value: `המספר החדש גדול מ־[[${n}]]`, misconception: "moves-right-instead-of-left" }, { value: `המספר החדש שווה ל־[[${n}]]`, misconception: "movement-does-not-change-value" }, { value: "אי אפשר לדעת", misconception: "avoids-number-line-direction" }], n + sampledInteger(params, "wordingVariant")); },
});
