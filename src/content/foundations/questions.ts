...
const GLOBAL_AUTHORING_GENERATORS = GLOBAL_BANDS.flatMap((band) => {
  ...
  return [
    ...
    conceptualGenerator({ id: `MVP_ALG_EQUALITY_COMMUTATIVE_${band}`, skillId: "ALG_EQUALITY", family: "commutative-equality", category, band, exprTemplate: "{a}+{b}={b}+{a}", params: pairParams, constraints: ["a != b"], difficultyFeature: "magnitude", choiceBuilder: (params) => { const a = sampledInteger(params, "a"); const b = sampledInteger(params, "b"); return generatedChoiceDraft("ALG_EQUALITY", `איזה משפט שוויון נכון עבור ${a} ו־${b}?`, [{ value: `${a} + ${b} = ${b} + ${a}`, correct: true, misconception: "commutative-equality" }, { value: `${a} + ${b} = ${a + b + 1}`, misconception: "accepts-near-result" }, { value: `${a} - ${b} = ${b} - ${a}`, misconception: "overgeneralizes-commutativity" }, { value: `${a} + 0 = 0`, misconception: "misuses-additive-identity" }], a + b); } }),
    conceptualGenerator({ id: `MVP_ALG_VARIABLE_COEFFICIENT_${band}`, skillId: "ALG_VARIABLE", family: "variable-in-term", category, band, exprTemplate: "{n}x", params: singleParams, difficultyFeature: "magnitude", choiceBuilder: (params) => { const n = sampledInteger(params, "n"); return generatedChoiceDraft("ALG_VARIABLE", `בביטוי ${n}x, מה מייצגת x?`, [{ value: "מספר שיכול לקבל ערך", correct: true, misconception: "variable-can-vary" }, { value: "סימן כפל בלבד", misconception: "reads-variable-as-operation" }, { value: `תמיד ${n}`, misconception: "confuses-variable-with-coefficient" }, { value: "סימן שוויון", misconception: "confuses-algebraic-symbols" }], n); } }),
    conceptualGenerator({ id: `MVP_ALG_SUBSTITUTE_LINEAR_${band}`, skillId: "ALG_SUBSTITUTE", family: "substitute-into-linear-expression", category, band, exprTemplate: "{a}*{x}+{b}", params: { a: { type: "natural", min: 2, max: 5 }, x: { type: "natural", ...range }, b: { type: "natural", min: 1, max: 6 } }, constraints: ["b != x", "a * x != a + x"], difficultyFeature: "magnitude", choiceBuilder: (params) => { const a = sampledInteger(params, "a"); const x = sampledInteger(params, "x"); const b = sampledInteger(params, "b"); return generatedChoiceDraft("ALG_SUBSTITUTE", `אם x = ${x}, איזה ביטוי מתאר ${a}x + ${b} לאחר ההצבה?`, [{ value: `${a} × ${x} + ${b}`, correct: true, misconception: "substitutes-variable-value" }, { value: `${a} + ${x} + ${b}`, misconception: "replaces-coefficient-multiplication-with-addition" }, { value: `${x} + ${b}`, misconception: "drops-coefficient" }, { value: `${a} × ${b} + ${x}`, misconception: "substitutes-into-wrong-position" }], a + x + b); } }),
    conceptualGenerator({ id: `MVP_EQ_ADD_MISSING_${band}`, skillId: "EQ_ADD", family: "missing-addend-equation", category, band, exprTemplate: "{a}+{b}={a}+{b}", params: pairParams, constraints: ["a != b"], difficultyFeature: "magnitude", choiceBuilder: (params) => { const a = sampledInteger(params, "a"); const b = sampledInteger(params, "b"); return generatedChoiceDraft("EQ_ADD", `איזה ערך משלים את השוויון □ + ${b} = ${a + b}?`, [{ value: `${a}`, correct: true, misconception: "inverse-addition" }, { value: `${-a}`, misconception: "uses-opposite-value" }, { value: `${a + b}`, misconception: "uses-result-as-unknown" }, { value: `${b}`, misconception: "reuses-visible-addend" }], a + b); } }),
    conceptualGenerator({ id: `MVP_EQ_MUL_MISSING_${band}`, skillId: "EQ_MUL", family: "missing-factor-equation", category, band, exprTemplate: "{b}*{a}={a}*{b}", params: pairParams, constraints: ["a != b"], difficultyFeature: "magnitude", choiceBuilder: (params) => { const a = sampledInteger(params, "a"); const b = sampledInteger(params, "b"); return generatedChoiceDraft("EQ_MUL", `איזה ערך משלים את השוויון ${b} × □ = ${a * b}?`, [{ value: `${a}`, correct: true, misconception: "inverse-multiplication" }, { value: `${-a}`, misconception: "uses-opposite-value" }, { value: `${a * b}`, misconception: "uses-product-as-factor" }, { value: `${b}`, misconception: "reuses-visible-factor" }], a * b); } }),
  ];
});
...
const CURATED_WORDING_ITEMS: SkillQuestionDefinition[] = [{
  ...
  difficultyBand: "A",
  difficulty: 0.12,
  prompt: [{ kind: "text", value: "אם n מייצג את מספר התלמידים בקבוצה, מה יכול להשתנות?" }],
  options: ["הערך של n", "האות n עצמה", "סימן השוויון", "שם הקבוצה"].map((value, index) => ({ id: `o${index}`, content: [{ kind: "text", value }], misconceptionId: index === 0 ? undefined : misconceptionPattern("ALG_VARIABLE", "reasoning", index), misconceptionRationale: index === 0 ? undefined : misconceptionRationale(misconceptionPattern("ALG_VARIABLE", "reasoning", index)) })),
  correctOptionId: "o0",
