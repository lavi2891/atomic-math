...
export function SessionSummaryScreen({ completed, masteryBefore, masteryAfter, personalBestUpdate, onHome, onRepeat }: Props) {
  ...
  const elapsedSeconds = Math.round((completed.elapsedDurationMs ?? 0) / 1000); const fixed = completed.session.settings.mode === "fixed";
  return <section style={{ display: "grid", gap: spacing.md }}>
    ...
    {fixed ? <div className="personal-best-result"><strong>{personalBestUpdate?.isNewRecord ? `שיא חדש: ${Math.round((personalBestUpdate.best?.bestScore ?? 0) / 1000)} שניות!` : `סיימת ב-${elapsedSeconds} שניות`}</strong>{personalBestUpdate && !personalBestUpdate.isNewRecord ? <span>השיא שלך: {Math.round((personalBestUpdate.best?.bestScore ?? 0) / 1000)} שניות</span> : null}<small>{accuracy < 90 ? `${correct}/${results.length} נכונות · צריך לפחות 90% דיוק כדי לקבוע שיא` : `דיוק: ${accuracy}% · השיא הוא שלך בלבד`}</small></div> : personalBestUpdate ? <div className="personal-best-result"><strong>{personalBestUpdate.isNewRecord ? `שיא חדש: ${personalBestUpdate.best?.bestScore ?? correct}!` : completed.session.settings.mode === "survival" ? `הגעת ל-${correct}` : `${correct} נכונות`}</strong>{!personalBestUpdate.isNewRecord ? <span>השיא שלך: {personalBestUpdate.best?.bestScore}</span> : null}<small>דיוק: {accuracy}% · השיא הוא שלך בלבד</small></div> : null}
