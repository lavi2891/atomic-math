...
import { DOMAINS, listSkillsByDomain } from "../../content/catalog/index.ts";
...
type Props = { domainId: string; definitions: readonly SkillQuestionDefinition[]; onBack: () => void; onStart: (skillIds: string[], settings: SessionSettings) => void };
const MODES: Array<{ id: SessionMode; icon: string; title: string; description: string }> = [
  { id: "fixed", icon: "#", title: "מספר שאלות", description: "בוחרים כמה לפתור" }, { id: "survival", icon: "♡", title: "הישרדות", description: "3 טעויות עד הסיום" },
  { id: "timed", icon: "◷", title: "נגד השעון", description: "3 דקות" }, { id: "practice", icon: "∞", title: "תרגול חופשי", description: "בלי הגבלה" },
];
...
export function SessionSetupScreen({ domainId, definitions, onBack, onStart }: Props) {
  ...
  function start() { if (!selected.length) return; const settings: SessionSettings = mode === "fixed" ? { mode, questionCount } : mode === "survival" ? { mode, maxErrors: sessionDefaults.survivalMaxErrors } : mode === "timed" ? { mode, durationSeconds: sessionDefaults.timedDurationSeconds } : { mode: "practice" }; onStart(selected, settings); }
  return <section className="setup-screen">
    ...
    <button type="button" className="primary-action" disabled={!selected.length} onClick={start}>התחלת תרגול</button>
  </section>;
}
