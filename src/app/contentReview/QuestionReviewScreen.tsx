      ...
      <aside className="review-panel review-sidebar review-details-pane">{currentDefinition && currentQuestion ? <>
        {showDetails ? <div className="review-details"><section className="review-human-summary"><h2>איך השאלה בנויה</h2><p><strong>{currentSkill?.nameHe}</strong> · {currentDefinition.category} · Band {currentDefinition.difficultyBand}</p><p><strong>סוג תשובה:</strong> {answerSemanticsLabel(currentQuestion)}</p>{familyNote?.rationaleHe ? <p><strong>מטרה פדגוגית:</strong> {familyNote.rationaleHe}</p> : null}
          {isGeneratedQuestionDefinition(currentDefinition) ? <>
            ...
            <h3>משתנים</h3><table><thead><tr><th>משתנה</th><th>ערכים אפשריים</th></tr></thead><tbody>{variables.map((variable) => <tr key={variable.name}><td><code>{variable.name}</code></td><td>{variable.valuesLabel}</td></tr>)}</tbody></table>
            <h3>הבדלים בין רמות</h3><div className="review-band-summaries">{bandSummaries.map((summary, summaryIndex) => <article key={summary.band} data-current={summary.band === currentDefinition.difficultyBand}><strong>רמה {summary.band}</strong>{structure && summary.template !== structure.template ? <div className="review-band-template"><small>{summary.templateChangedFromPrevious ? "התבנית משתנה ברמה זו:" : "תבנית ברמה זו:"}</small><ReviewMath latex={summary.template} /></div> : null}{summaryIndex === 0 ? <>{summary.variables.map((variable) => <small key={variable.name}><code>{variable.name}</code>: {variable.valuesLabel}</small>)}{summary.constraints.humanReadable.length ? <small>תנאים: {summary.constraints.humanReadable.join("; ")}</small> : null}</> : summary.changesFromPrevious.length ? <ul>{summary.changesFromPrevious.map((change) => <li key={change}>{change}</li>)}</ul> : <small>ללא שינוי בטווחים או בתנאים</small>}</article>)}</div>
