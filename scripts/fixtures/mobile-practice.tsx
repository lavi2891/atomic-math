...
const choiceType = params.get("choice");
const choiceOptions = ["a", "b", "c", "d"].map(id => ({ id, content: [{ kind: "text" as const, value: `אפשרות ${id}` }] }));
const definitions = choiceType === "single" ? [{ id: "single-choice-fixture", topicId: "FOUNDATIONS", skillId: skill.id, type: "singleChoice" as const, difficultyBand: "A" as const, tags: ["short-item"], prompt: [{ kind: "text" as const, value: "בחרו תשובה אחת" }], options: choiceOptions.slice(0, 3), correctOptionId: "b" }]
  : choiceType === "multi" ? [{ id: "multi-choice-fixture", topicId: "FOUNDATIONS", skillId: skill.id, type: "multiChoice" as const, difficultyBand: "A" as const, tags: ["short-item"], prompt: [{ kind: "text" as const, value: "בחרו את כל התשובות הנכונות" }], options: choiceOptions, correctOptionIds: ["a", "c"] }]
  : skills.filter(item => !params.has("missing") || item.id !== "INT_ADD").map(item => ({ id: item.id + "-fixture", topicId: "FOUNDATIONS", skillId: item.id, type: "numeric" as const, difficultyBand: "A" as const, tags: ["short-item"], prompt: [{ kind: "text" as const, value: params.has("long") ? "שאלה ארוכה לבדיקה. ".repeat(100) : "חשבו: " }, { kind: "math" as const, latex: "1 + 1" }], correctAnswers: ["2"] as [string], acceptedInputFormats: ["integer" as const] }));
