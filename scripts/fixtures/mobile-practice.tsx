...
export function Fixture() {
  const [session] = useState(() => ({ id: crypto.randomUUID(), studentId: "mobile-test", selectedSkillIds: skills.map(item => item.id), settings, startedAt: Date.now() - (params.has("missing") ? 24000 : 0), ...(params.has("stage") || completion ? { learningStage: { pathId: "NUMBERS_ALGEBRA" as const, stageId: "NA_PLACE_VALUE" } } : {}) }));
  const [completed, setCompleted] = useState<PracticeSessionState | undefined>(() => completion ? {
    ...
    results: Array.from({ length: 5 }, (_, index) => ({ questionId: `summary-${index}`, topicId: "FOUNDATIONS", attemptIndex: 0, isCorrect: completion === "passed" || index < 2, rawAnswer: { questionType: "numeric" as const, data: { value: completion === "passed" ? "2" : "0" } }, responseTimeMs: 1000, timestamp: Date.now(), questionSnapshot: { ...definitions[0]!, id: `summary-${index}` } })),
  } : undefined);
  return <div className="page" dir="rtl" style={styles.page}><div className="phone" style={styles.phone}>
    {completed ? <SessionSummaryScreen completed={completed} masteryBefore={{}} masteryAfter={{}} personalBestUpdate={null} stageStars={completion === "passed" ? 3 : completion === "retry" ? 0 : undefined} previousStageBestStars={0} totalStarsBefore={34} onHome={() => {}} onRepeat={() => {}} /> :
