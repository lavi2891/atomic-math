...
const fixtureStudentId = "HOME-TEST";
...
for (const stage of stages) {
  await sessionRepository.saveSession({
    id: `fixture-${stage.id}`, studentId: fixtureStudentId,
...
if (new URLSearchParams(location.search).get("shortcut") === "passed" && firstChapter.shortcutTest) {
  await sessionRepository.saveSession({
    id: "fixture-shortcut", studentId: fixtureStudentId, selectedSkillIds: [...firstChapter.shortcutTest.skillIds],
...
if (new URLSearchParams(location.search).has("best")) {
  await personalBestRepository.record({ studentId: fixtureStudentId, signature: { mode: "fixed", questionCount: 5, scope: { type: "skill", skillId: "AR_PLACE_VALUE" } }, bestScore: 10_000, achievedAt: new Date().toISOString(), sessionId: "fixture-best", metrics: { attempted: 5, correct: 5, incorrect: 0, accuracy: 1, durationMs: 10_000 } });
}
