...
run("student bootstrap distinguishes active, inactive, and unknown codes without returning a roster",()=>{const source=readFileSync(new URL("../apps-script/Code.gs",import.meta.url),"utf8");assert.match(source,/studentStatus:student\?\(active\?"active":"inactive"\):"unknown"/);assert.match(source,/activeAssignments:active\?/);assert.doesNotMatch(source,/return\{students:/);});
